...
msoa_data %>%
  ...
  add_count(subgroup_code, subgroup_name, name = "n_oas") %>%
...
The following chart shows the case rates for weeks 36-42 by area type but again with a varying y-axis per group and a dashed horizontal line to indicate a rate of 100 cases per 100,000.

```{r}

ggplot(filter(type_rate_graph_dt, week >= 35), aes(x = week, y = rate, colour = subgroup)) +
  geom_hline(yintercept = 100, linetype = "dashed", size = 0.3, colour = "grey40") +
  geom_line() +
  labs(x = "Week", y = "Covid-19 case rate (per 100,000)") +
...
There still some area types where the case rate is below 100, but not many and the majority of places appear to be on a trajectory towards breaching that rate.
